...
kernel1 = tf.nn.conv2d(image_holder, weight1, [1,2, 2, 1], padding='SAME',name='1')
bias1 = tf.Variable(tf.constant(0.0, shape=[64]))
# conv1 = tf.nn.relu(tf.nn.bias_add(kernel1,bias1))
conv1 = tf.nn.relu(kernel1 + bias1)    #12*12*64
pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')
norm1 = tf.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,name='11')    #6*6*64
...
#
# #全链接
reshape = tf.reshape(norm1, [batch_size, -1])  #分开成为
dim = reshape.get_shape()[1].value       #
weight3 = weight_variable([dim, 10])
bias3 = tf.Variable(tf.constant(0.1, shape=[10]))
local3 = tf.nn.relu(tf.matmul(reshape, weight3) + bias3)
# #全连接层
# weight4 = weight_variable([64, 192])
# bias4 = tf.Variable(tf.constant(0.1, shape=[192]))
# local4 = tf.nn.relu(tf.matmul(conv2, weight4) + bias4)
#输出层
# weight5 = weight_variable([192, 10])
# bias5 = tf.Variable(tf.constant(0.0, shape=[10]))
# logits = tf.matmul(local3, weight5) + bias5
...
print('v_list',v_list)
print('g_list',g_list)
print('v_norm',v_norms)
print('g_norms',g_norms)
print('zeds',zeds)


top_k_op = tf.nn.in_top_k(local3, tf.cast(label_holder, tf.int64), 1)
...
for step in range(max_steps):
    ...
    _, loss_value,v,g,vv,gg,zz = sess.run([grads_and_vars, loss,v_list,g_list,v_norms,g_norms,zeds], feed_dict={image_holder: image_batch, label_holder: label_batch})
